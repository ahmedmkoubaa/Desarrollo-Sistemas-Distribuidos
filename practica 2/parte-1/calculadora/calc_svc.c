/*
 * Please do not edit this file.
 * It was generated using rpcgen.
 */

#include "calc.h"
#include <stdio.h>
#include <stdlib.h>
#include <rpc/pmap_clnt.h>
#include <string.h>
#include <memory.h>
#include <sys/socket.h>
#include <netinet/in.h>

#ifndef SIG_PF
#define SIG_PF void(*)(int)
#endif

static double *
_sumar_1 (Operacion  *argp, struct svc_req *rqstp)
{
	return (sumar_1_svc(*argp, rqstp));
}

static double *
_restar_1 (Operacion  *argp, struct svc_req *rqstp)
{
	return (restar_1_svc(*argp, rqstp));
}

static double *
_multiplicar_1 (Operacion  *argp, struct svc_req *rqstp)
{
	return (multiplicar_1_svc(*argp, rqstp));
}

static double *
_dividir_1 (Operacion  *argp, struct svc_req *rqstp)
{
	return (dividir_1_svc(*argp, rqstp));
}

static double *
_media_vector_1 (Vector  *argp, struct svc_req *rqstp)
{
	return (media_vector_1_svc(*argp, rqstp));
}

static Vector *
_multiplicar_escalar_1 (multiplicar_escalar_1_argument *argp, struct svc_req *rqstp)
{
	return (multiplicar_escalar_1_svc(argp->arg1, argp->arg2, rqstp));
}

static long *
_sumatoria_1 (int  *argp, struct svc_req *rqstp)
{
	return (sumatoria_1_svc(*argp, rqstp));
}

static void
calcprog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		Operacion sumar_1_arg;
		Operacion restar_1_arg;
		Operacion multiplicar_1_arg;
		Operacion dividir_1_arg;
		Vector media_vector_1_arg;
		multiplicar_escalar_1_argument multiplicar_escalar_1_arg;
		int sumatoria_1_arg;
	} argument;
	char *result;
	xdrproc_t _xdr_argument, _xdr_result;
	char *(*local)(char *, struct svc_req *);

	switch (rqstp->rq_proc) {
	case NULLPROC:
		(void) svc_sendreply (transp, (xdrproc_t) xdr_void, (char *)NULL);
		return;

	case SUMAR:
		_xdr_argument = (xdrproc_t) xdr_Operacion;
		_xdr_result = (xdrproc_t) xdr_double;
		local = (char *(*)(char *, struct svc_req *)) _sumar_1;
		break;

	case RESTAR:
		_xdr_argument = (xdrproc_t) xdr_Operacion;
		_xdr_result = (xdrproc_t) xdr_double;
		local = (char *(*)(char *, struct svc_req *)) _restar_1;
		break;

	case MULTIPLICAR:
		_xdr_argument = (xdrproc_t) xdr_Operacion;
		_xdr_result = (xdrproc_t) xdr_double;
		local = (char *(*)(char *, struct svc_req *)) _multiplicar_1;
		break;

	case DIVIDIR:
		_xdr_argument = (xdrproc_t) xdr_Operacion;
		_xdr_result = (xdrproc_t) xdr_double;
		local = (char *(*)(char *, struct svc_req *)) _dividir_1;
		break;

	case MEDIA_VECTOR:
		_xdr_argument = (xdrproc_t) xdr_Vector;
		_xdr_result = (xdrproc_t) xdr_double;
		local = (char *(*)(char *, struct svc_req *)) _media_vector_1;
		break;

	case MULTIPLICAR_ESCALAR:
		_xdr_argument = (xdrproc_t) xdr_multiplicar_escalar_1_argument;
		_xdr_result = (xdrproc_t) xdr_Vector;
		local = (char *(*)(char *, struct svc_req *)) _multiplicar_escalar_1;
		break;

	case SUMATORIA:
		_xdr_argument = (xdrproc_t) xdr_int;
		_xdr_result = (xdrproc_t) xdr_long;
		local = (char *(*)(char *, struct svc_req *)) _sumatoria_1;
		break;

	default:
		svcerr_noproc (transp);
		return;
	}
	memset ((char *)&argument, 0, sizeof (argument));
	if (!svc_getargs (transp, (xdrproc_t) _xdr_argument, (caddr_t) &argument)) {
		svcerr_decode (transp);
		return;
	}
	result = (*local)((char *)&argument, rqstp);
	if (result != NULL && !svc_sendreply(transp, (xdrproc_t) _xdr_result, result)) {
		svcerr_systemerr (transp);
	}
	if (!svc_freeargs (transp, (xdrproc_t) _xdr_argument, (caddr_t) &argument)) {
		fprintf (stderr, "%s", "unable to free arguments");
		exit (1);
	}
	return;
}

int
main (int argc, char **argv)
{
	register SVCXPRT *transp;

	pmap_unset (CALCPROG, CALCVER);

	transp = svcudp_create(RPC_ANYSOCK);
	if (transp == NULL) {
		fprintf (stderr, "%s", "cannot create udp service.");
		exit(1);
	}
	if (!svc_register(transp, CALCPROG, CALCVER, calcprog_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (CALCPROG, CALCVER, udp).");
		exit(1);
	}

	transp = svctcp_create(RPC_ANYSOCK, 0, 0);
	if (transp == NULL) {
		fprintf (stderr, "%s", "cannot create tcp service.");
		exit(1);
	}
	if (!svc_register(transp, CALCPROG, CALCVER, calcprog_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (CALCPROG, CALCVER, tcp).");
		exit(1);
	}

	svc_run ();
	fprintf (stderr, "%s", "svc_run returned");
	exit (1);
	/* NOTREACHED */
}
